...
public class MoveSetScript : MonoBehaviour {
    ...
    public void ChangeMoveStances(CombatStances newStance)
    {
        ...
        foreach (MoveSetData moveSetData in controlsScript.loadedMoves) {
            if (moveSetData.combatStance == newStance) {
                ...
                if (moveSetData.roundOutro != null) {
                    roundOutro = Instantiate(moveSetData.roundOutro) as MoveInfo;
                    roundOutro.name = "Round Outro";
                    attachAnimation(roundOutro.animMap.clip, roundOutro.name, roundOutro._animationSpeed, roundOutro.wrapMode, roundOutro.animMap.length);
                }
                if (moveSetData.timeOutOutro != null)
                {
                    timeOutOutro = Instantiate(moveSetData.timeOutOutro) as MoveInfo;
                    timeOutOutro.name = "Time Out Outro";
                    attachAnimation(timeOutOutro.animMap.clip, timeOutOutro.name, timeOutOutro._animationSpeed, timeOutOutro.wrapMode, timeOutOutro.animMap.length);
                }
                if (moveSetData.cinematicOutro != null)
                {
                    gameOutro = Instantiate(moveSetData.cinematicOutro) as MoveInfo;
                    gameOutro.name = "Game Outro";
                    attachAnimation(gameOutro.animMap.clip, gameOutro.name, gameOutro._animationSpeed, gameOutro.wrapMode, gameOutro.animMap.length);
                }
    ...
    public bool ValidateMoveStances(PlayerConditions conditions, ControlsScript cScript, bool bypassCrouchStance = false)
    {
        ...
        foreach (PossibleMoveStates possibleMoveState in conditions.possibleMoveStates)
        {
            ...
            /*if (!possibleMoveState.blocking && !UFE.config.blockOptions.allowMoveCancel && cScript.blockStunned == possibleMoveState.blockStunned
                && (cScript.currentSubState == SubStates.Blocking || cScript.isBlocking)) continue;

			if (!possibleMoveState.stunned && possibleMoveState.possibleState != PossibleStates.Down && cScript.currentSubState == SubStates.Stunned) continue;

            if (!possibleMoveState.stunned && !possibleMoveState.resetStunValue && cScript.stunTime > 0) continue;*/
	...
	private MoveInfo TestMoveExecution(MoveInfo move, MoveInfo currentMove, ButtonPress[] buttonPress, bool inputUp, bool fromSequence, bool forceExecution)
    {
        ...
        if (move.projectiles.Length > 0 && controlsScript.projectiles.Count > 0) {
            ...
            {
                if (pScript == null || pScript.IsDestroyed()) continue;
