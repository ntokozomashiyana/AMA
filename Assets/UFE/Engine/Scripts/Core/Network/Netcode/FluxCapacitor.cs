...
public class FluxCapacitor {
	...
	public virtual void Initialize(long currentFrame, int maxHistoryLength)
    {
		...
		if (!UFE.isConnected) maxHistoryLength = UFE.config.networkOptions.defaultFrameDelay;
    ...
    public virtual void ApplyInputs(long currentFrame){
        ...
        if (!UFE.isPaused()) 
		{
			...
			foreach (ControlsScript cScript in allScripts)
			{
				if (!UFE.freezePhysics && cScript != null && cScript.GetActive())
				{
					foreach (ControlsScript assist in cScript.assists)
					{
						assist.CheckHits(assist.currentMove, assist.opControlsScript);
						foreach (ControlsScript opAssist in assist.opControlsScript.assists)
                        {
							assist.CheckHits(assist.currentMove, opAssist);
						}
					}
					cScript.CheckHits(cScript.currentMove, cScript.opControlsScript);
				}
			}
	...
	protected virtual void ForceDisconnection(string disconnectionCause = null){
...
//		}
    ...
    protected virtual void UpdateInstantiatedObjects(long currentFrame)
    {
        foreach (InstantiatedGameObject entry in UFE.instantiatedObjects.ToArray())
        {
			...
			bool wasActive = entry.gameObject.activeInHierarchy;
			...
			if (entry.mrFusion != null && entry.gameObject.activeInHierarchy != wasActive) entry.mrFusion.DestroyEvent();
