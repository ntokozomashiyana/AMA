...
namespace UFE3D
{
    public class QuickMatchScreen : UFEScreen
    {
        ...
        protected virtual void OnMatchesDiscovered(ReadOnlyCollection<MultiplayerAPI.MatchInformation> matches)
        {
            ...
            if (matches != null)
            {
                ...
                {
                    if (matches[i] != null && !matches[i].matchName.Contains("RoomMatch_"))
                    {
        ...
        protected virtual void TryConnect()
        {
            ...
            if (!UFE.multiplayerAPI.IsConnectedToGame() && !this._connecting)
            {
